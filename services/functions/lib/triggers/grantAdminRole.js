...
exports.grantAdminRole = functions.https.onCall(async (data, context) => {
    ...
    const user = await admin_1.admin.auth().getUserByEmail(data.email);
    await admin_1.admin.auth().setCustomUserClaims(user.uid, { admin: true });
    return { success: true };
});
//# sourceMappingURL=grantAdminRole.js.map